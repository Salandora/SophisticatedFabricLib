 ...
 */
package com.github.salandora.sophisticatedfabriclib.common.api.v1.client.gui.controls;
...
public abstract class ScrollPanel extends AbstractContainerEventHandler implements Renderable, NarratableEntry {
	...
	protected void drawBackground(GuiGraphics guiGraphics, Tesselator tess, float partialTick) {
		...
		if (this.client.level != null) {
			guiGraphics.fillGradient(this.left, this.top, this.right, this.bottom, bgColorFrom, bgColorTo);
		}
	...
	public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
		...
		if (extraHeight > 0) {
			...
			worldr.vertex(barLeft, this.bottom, 0.0F).color(barBgRed, barBgGreen, barBgBlue, barBgAlpha).endVertex();
			worldr.vertex(barLeft + barWidth, this.bottom, 0.0F).color(barBgRed, barBgGreen, barBgBlue, barBgAlpha).endVertex();
			worldr.vertex(barLeft + barWidth, this.top, 0.0F).color(barBgRed, barBgGreen, barBgBlue, barBgAlpha).endVertex();
			worldr.vertex(barLeft, this.top, 0.0F).color(barBgRed, barBgGreen, barBgBlue, barBgAlpha).endVertex();
			tess.end();
			...
			worldr.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR);
			worldr.vertex(barLeft, barTop + barHeight, 0.0F).color(barRed, barGreen, barBlue, barAlpha).endVertex();
			worldr.vertex(barLeft + barWidth, barTop + barHeight, 0.0F).color(barRed, barGreen, barBlue, barAlpha).endVertex();
			worldr.vertex(barLeft + barWidth, barTop, 0.0F).color(barRed, barGreen, barBlue, barAlpha).endVertex();
			worldr.vertex(barLeft, barTop, 0.0F).color(barRed, barGreen, barBlue, barAlpha).endVertex();
			tess.end();
			...
			worldr.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR);
			worldr.vertex(barLeft, barTop + barHeight - 1, 0.0F).color(barBorderRed, barBorderGreen, barBorderBlue, barBorderAlpha).endVertex();
			worldr.vertex(barLeft + barWidth - 1, barTop + barHeight - 1, 0.0F).color(barBorderRed, barBorderGreen, barBorderBlue, barBorderAlpha).endVertex();
			worldr.vertex(barLeft + barWidth - 1, barTop, 0.0F).color(barBorderRed, barBorderGreen, barBorderBlue, barBorderAlpha).endVertex();
			worldr.vertex(barLeft, barTop, 0.0F).color(barBorderRed, barBorderGreen, barBorderBlue, barBorderAlpha).endVertex();
			tess.end();
		}
