package com.github.salandora.sophisticatedfabriclib.model.impl.v1.client;
...
import com.github.salandora.sophisticatedfabriclib.model.api.v1.models.BlockModelWrapper;
import com.github.salandora.sophisticatedfabriclib.model.mixin.client.accessors.BlockModelAccessor;
					...
					try (Reader reader = entry.getValue().openAsReader()) {
						...
						if (loader != null) {
							if (element.has("transform")) {
								SophisticatedFabricLib.LOGGER.info("Found transform element in {}", entry.getKey());
							}

							if (element.has("visibility")) {
								SophisticatedFabricLib.LOGGER.info("Found visibility element in {}", entry.getKey());
							}

							return Pair.of(id, loader.read(element));
						}
					} catch(IOException e) {
						SophisticatedFabricLib.LOGGER.error("Failed to load model {}", entry.getKey(), e);
					}
					return null;
				}, executor));
			}
